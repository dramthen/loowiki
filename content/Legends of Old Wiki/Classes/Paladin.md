The Paladin is a sturdy melee combatant with access to various Sigils. Sigils empower your attacks with various effects and can be detonated with the spell Holy Blinding for an immediate effect based on the sigil along with an instance of Pure damage.

Paladin has many support options available and can uniquely take on encounters with their spells Chains of Eltair, Lull, Sigil of Redemption, and Sigil of Bulwarking. Paladins also have the ability to equip shields, giving them an additional method of avoiding melee damage no other class has access to.

Paladins innately have a chance to double attack on every melee swing. They can also start off wearing cloth and leather while at higher levels unlocking chain and plate [[Combat Specifics|armor]]. Wields one-handed swords, clubs, or axes and a shield.

- **Melee Styles**
	 - Enrage
		 - Endurance: 25~
		 - Effect: Increases damage by a small amount and creates more threat
		 - Requirement?: None
	 - Retaliate
		 - Endurance: 25~
		 - Effect: Increases damage, allows Retribution activation
		 - Requirement?: Only after a Parry
	 - Retribution
		 - Endurance: 25~
		 - Effect: Increases damage and applies a snare for 18 seconds
		 - Requirement?: Only after a successful Retaliate
	 - Slam
		 - Endurance: 25~
		 - Effect: Increases damage and stuns the target for 4 seconds
		 - Requirement?: Only after a Block
	 - Luminael's Vengeance
		 - Endurance: 20~
		 - Effect: Increased damage, allows Luminael's Blessing activation
		 - Requirement?: Only from the side (with enough Agility and a decent weapon, this can be done in solo play after a Slam!)
	 - Luminael's Blessing
		 - Endurance: 20~
		 - Effect: Increased damage, provides a small group heal
		 - Requirement?: Only after a successful Luminael's Vengeance
- **Sigils (Justice)**
	- Sigil of Atonement
		- Mana: 20~
		- Duration: Until Detonated with Holy Blinding
		- Effect: Applies lightning damage to your target on Melee strike
	- Sigil of Redemption
		- Mana: 20~
		- Duration: Until Detonated with Holy Blinding
		- Effect: Applies small heal to party on Melee strike
	- Sigil of Rebuke
		- Mana: 20~
		- Duration: Until Detonated with Holy Blinding
		- Effect: Applies a fire based damage shield to yourself, burning attacking enemies
	- Sigil of Bulwarking
		 - Mana: 20~
		 - Duration: Until Detonated with Holy Blinding
		 - Effect: Increases your chance to block incoming attacks
- **Spells (Conviction)**
	- Holy Armor
		- Mana: 20~
		- Duration: 35 seconds
		- Cooldown: 60 seconds
		- Effect: A self only buff that increases your absorbtion by a large amount
	- Chains of Eltair
		- Mana: 20~
		- Duration: 18 seconds
		- Cooldown: 12 seconds
		- Effect: Attempts to push the target backwards (of their own faced duration), and applies a very powerful snaring effect
	- Lay on Hands
		- Mana: 20~
		- Duration: 3 ticks
		- Cooldown: 300 seconds
		- Effect: Provides an immediate heal and a lingering heal over time.
	- Lull
		- Mana: 15~
		- Duration: 8 seconds (unconfirmed)
		- Cooldown: 300 seconds
		- Effect: Removes the sight and social aggro of an enemy. Allowing you to pass by, or pull others around it without it assisting.
	- Holy Blinding
		- Mana: 25~
		- Cooldown: 15~ seconds
		- Effect: Deals instant pure damage to the target
		- Active Sigil determines additional effects:
			- Atonement: Additional Lightning Damage
			- Redemption: Additional Group Heal
			- Rebuke: AE Fire Damage
			- Bulwarking: Full Endurance Restoration for Party

[[Classes|Back to Classes]]
[[Legends Of Old|Back to Main]]